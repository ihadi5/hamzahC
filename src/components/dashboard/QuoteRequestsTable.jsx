...
export default function QuoteRequestsTable() {
  ...
  useEffect(() => {
    base44.entities.QuoteRequest.list('-created_date').then(data => { setRequests(data); setLoading(false); });
  }, []);
      ...
      {selected && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setSelected(null)}>
          <div className="bg-white rounded-3xl p-8 max-w-lg w-full shadow-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <button onClick={() => setSelected(null)} className="mt-8 w-full py-3 rounded-xl bg-slate-100 text-slate-600 font-bold hover:bg-slate-200 transition-colors">
              إغلاق
            </button>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm">
        <div className="p-6 border-b border-slate-100">
          <h3 className="font-bold text-[#0f1b2d] flex items-center gap-2">
            ...
            طلبات التسعيرة ({requests.length})
          </h3>
        </div>
              ...
              <tbody className="divide-y divide-slate-50">
                {requests.map(r => (
                  <tr key={r.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <p className="text-slate-600 font-medium" dir="ltr">{r.phone}</p>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1.5 rounded-full text-xs font-bold ${statusColor[r.status] || 'bg-slate-100'}`}>
                        {r.status}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <button 
                        ...
                        className="p-2.5 rounded-lg bg-orange-50 text-orange-500 hover:bg-orange-100 hover:shadow-sm transition-all"
                        title="عرض التفاصيل"
                      >
